...
`ensureReady(target, { signal, deadline })` 是所有远程服务的统一前置步骤：
...
6. 多个并发调用共享同一个连接 Promise；等待者可以独立取消，只有最后一个等待者离开时才取消共享建链。
7. 返回包含 generation token 的 connection lease 和建链耗时。
...
- cold 启动发现既有 ControlPath 时必须先执行 `ssh -O check`；健康 socket 直接接管，失效 socket 才删除重建。
- check/remove/open 使用同路径的原子锁目录协调多个 MCP 进程；锁记录 PID、实例 ID 和时间戳，死亡持有者或超时的无主锁可被回收。
- lease 带有本地 generation 与共享 generation token；失效通知只能淘汰发放该 lease 的 master，不能删除其他实例已经重建的新连接。
- 配置指纹改变后不再复用旧 master。
...
- 只读 file read/stat 与 download 在 mux 失败后安全重试一次。

所有 SSH/SCP 结果都检查 `mux_client`、`Session open refused`、`ControlSocket already exists` 等本地诊断，不依赖退出码 255：

- 命令 completed：保留成功结果，淘汰 master，并返回 `SSH_MASTER_DEGRADED` warning。
- 命令确认未 started：重建后最多重试一次。
- 命令已 started 但未 completed：返回 `mayHaveRun=true`，禁止重放。
- write、append、upload 与 detach 启动状态不明：禁止自动重放。

已识别的本地 mux 行从远程 stderr 中剥离，避免把本地连接故障伪装成远程程序输出。
...
- 每次 ssh/scp 调用创建独立本地进程组；Linux 还通过 `/proc` 递归收集并终止自行换组的 ProxyJump 后代。
- POSIX 取消先向进程组发送 `SIGTERM`，1 秒后仍未退出则发送 `SIGKILL`；Windows 使用 `taskkill /t` 回退终止进程树。
- Adapter 维护活动子进程注册表；正常 close 必须注销进程并清除升级定时器，避免 PID 复用后误杀。
- mux client 被终止后，持久 master 可能继续持有 stdio 管道；取消路径在 child `exit` 时主动断开本地读端，不等待 `close`，随后立即执行远程 PGID 清理。
- signal/deadline 贯穿凭据准备、`ssh -G`、跳板诊断、master check/open/close、exec 与 scp。

### 8.3 调用 deadline

公开 schema 不变。`timeoutMs` 表示整个调用从开始到返回的总预算，覆盖解析、建链、执行和清理。前序阶段消耗的时间会从最终 ssh/scp 的剩余时间中扣除；前台 exec 预留有限清理预算，用于在超时后终止已登记的远程进程组。
...
- 密码不进入 argv 和返回值。
- 取消和超时行为。
- 进程组 TERM/KILL 与活动注册表清空。
- 健康旧 socket 接管、stale 单次重建、跨实例 ControlPath 锁。
- generation 防止旧 lease 淘汰新 master。
- mux code=0 完成不重放、未 started 单次重试、started 后禁止重放。

### 19.3 Docker 集成测试

真实运行 OpenSSH 的单跳、双跳、三级目标、认证失败、host key 失败、网络断开和 master 损坏场景；同时覆盖双 MCP 实例、取消远程 sleep 后无残留，以及 `SIGSTOP`/`SIGCONT` 后强制健康复检。

### 19.4 MCP 服务生命周期

- stdio 关闭以及 `SIGTERM`、`SIGINT`、`SIGHUP` 触发统一 shutdown：取消活动请求，终止并等待本地 ssh/scp 进程退出，但按 ADR-003 保留健康的持久 ControlMaster。
- 收到 `SIGCONT` 时先向已登记活动进程组发送 `SIGCONT`，再把所有连接标记为待复检；下一次操作不得直接使用 TTL 内缓存。
- `SIGSTOP` 无法被进程捕获。宿主若不发送 `SIGCONT`，MCP 服务无法自救，客户端仍应负责检测失联并重启 connector。
