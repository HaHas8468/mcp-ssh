...
function createV3Runtime(overrides = {}) {
  ...
  const adapter = overrides.adapter || new OpenSshAdapter({ config });
  ...
  return {
    paths, config, adapter, catalog, resolver, connections, outputStore, taskStore, policy, audit,
...
async function requestApproval(server, { target, danger }) {
  try {
...
async function createServer({ runtime = createV3Runtime(), Server: ServerOverride } = {}) {
  ...
  const registry = new Map([
    ['ssh_targets', (args) => runtime.targets.handle(args)],
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request, extra = {}) => {
    ...
    const result = await handler(request.params.arguments || {}, {
      signal: extra.signal,
      onProgress,
      requestApproval: input => requestApproval(server, input),
    });
    runtime.audit?.log?.({
...
async function main() {
  try {
    ...
    try {
      ...
      watch(dirname(source), () => { runtime.catalog.invalidate(); runtime.config.invalidate(); });
