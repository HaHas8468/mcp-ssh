...
class TargetService {
  ...
  async handle(args = {}, context = {}) {
    ...
    try {
      if (action === 'list') {
        const targets = await this.catalog.list({ signal: context.signal });
      ...
      const target = await this.resolver.resolve(args.target, { signal: context.signal });
      if (action === 'describe') {
      ...
      if (args.networkProbe) {
        ...
        for (const hop of probes) {
          const result = await this.adapter.probe(hop.alias, { timeoutMs: 10_000, signal: context.signal });
