...
class TransferService {
  ...
  async handle(args = {}, context = {}) {
    ...
    try {
      ...
      return successResult({ requestId, operation: `transfer.${args.action}`, target: target.id, timing, data: { localPath, remotePath: args.remotePath, recursive: Boolean(args.recursive), bytesTransferred } });
    } catch (error) {
      timing.totalMs = Date.now() - startedAt;
      return failureResult({ requestId, operation: `transfer.${args.action || 'unknown'}`, target: target?.id || args.target, timing, error });
    }
  }
}
